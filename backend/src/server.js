  ...
  const content = [
    {
      ...
      text: [
        ...
        'The STRUCTURE TRANSCRIPTION must state: canvas orientation and ratio; exact visible panel count; a recursive split tree beginning with the whole page; every divider as vertical, horizontal, vertical-oblique or horizontal-oblique; its tilt direction; approximate normalized endpoints/intersections or percentage position; relative widths/heights or area fractions of the resulting regions; gutter thickness; reading order; and confidence.',
        'Describe nested divisions in spatial language such as: whole page split into left/right regions; then the right region split into upper/lower regions; then one child region split again. Never flatten a nested structure into a vague list of panels.',
        'Use fractions or percentages when visually defensible (for example 1/2, 1/5 versus 4/5, 1/4 versus 3/4). If a measurement is uncertain, label it approximate instead of inventing false precision.',
        'Ignore drawings, text, characters and style when an image is a blank structure template; analyze only its panel geometry.',
        'If the structure image is unclear or does not contain reliable panel separators, explicitly state LOW STRUCTURE CONFIDENCE and defer to the user prompt.',
...
function buildCompactMangaImagePrompt(context, maxLength = openAIImagePromptTargetLength) {
  ...
  const build = (budgets) =>
    joinPromptLines([
      ...
      'For a clear Storyboard/structure image, follow its self-contained STRUCTURE TRANSCRIPTION exactly: panel count, recursive split tree, divider orientation/tilt/intersections, relative proportions, gutters and reading order. Never substitute a generic grid. Explicit user geometry overrides conflicts; ambiguous or low-confidence image geometry defers to user text.',
      '',
...
function buildMangaImagePrompt(input) {
  ...
  const fullPrompt = joinPromptLines([
      ...
      '',
      'STRUCTURE GEOMETRY LOCK:',
      'When a clear Storyboard / structure image is supplied, reconstruct its panel grid from the written STRUCTURE TRANSCRIPTION: exact panel count, recursive parent/child splits, divider orientation and tilt, intersections, relative proportions, gutters, hierarchy and reading order. Do not replace it with a generic equal grid.',
      'The structure description must be operational and self-sufficient: another artist must be able to redraw the empty panel layout from the text alone without seeing the reference.',
      'Explicit user text has final authority over every conflicting structural detail. If the image is absent, ambiguous, low-confidence, or less precise than the user description, use the user description. If the user gives no contradictory geometry and the structure image is clear, the transcribed image geometry is mandatory.',
      '',
      'IMAGE ROLE INVENTORY:',
      ...
      'Storyboard or generated page references define HOW the page is organized: exact panel count, recursive divisions, divider angles and intersections, relative panel proportions, gutters, reading order, framing, camera angle, action order, character placement, and spatial relationships. A clear structure must be geometrically transcribed and followed, never approximated as a generic grid.',
      ...
      'Before generating, resolve these points from the user prompt, image roles, and reference analysis: task type, involved characters, reference image assigned to each character, character role in the scene, exact panel count, recursive panel split tree, divider orientation/tilt/intersections, relative panel proportions, gutters and reading order, panel function, character position per panel, expression per panel, pose/action mechanics, camera angle, dialogue text, style mode, background level, and what must remain faithful versus what may change.',
